...
def hierarchical_train_model(
    ...
    Returns:
        Optimized theta_pop (shape: [n_params]) and theta_subjects (shape: [n_subjects, n_params]).
    """
    ...
    params = init_params
    for i in trange(num_steps, desc="Hierarchical Training"):
        params, loss, opt_state = step_fn(params, opt_state)
        if verbose and (i % 100 == 0):
            print(f"Step {i:4d}, Hierarchical NLL: {loss:.4f}")
    theta_pop_opt = params[:n_params]
    theta_subjects_opt = params[n_params:].reshape(-1, n_params)
    return theta_pop_opt, theta_subjects_opt
...
def multi_start_hierarchical_train(
    ...
    Returns:
        Best theta_pop, best theta_subjects, and a list of final hierarchical losses for each restart.
    """
    best_theta_pop = None
